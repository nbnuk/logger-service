...
class LoggerController {
    ...
    /**
     * Generate a CSV file containing download statistics by month
     * <p/>
     * The request is expected to have the following parameters:
     * <ul>
     *     <li>eventId - the logEventTypeId to query on (defaults to 1002 for downloads). Optional.
     *     <li>fromMonth - the starting month in format yyyyMM (inclusive). Mandatory.
     *     <li>toMonth - the ending month in format yyyyMM (inclusive). Mandatory.
     *     <li>excludeReasonTypeId - the reason type ID to consider as "testing" (defaults to 10). Optional.
     * </ul>
     * Example request: <pre>.../logger/downloadsByMonthCSV?fromMonth=202104&toMonth=202203</pre>
     *
     * @return download statistics by month in CSV format
     */
    @Operation(
            method = "GET",
            tags = "logger",
            operationId = "Get Downloads By Month",
            summary = "Get Downloads By Month",
            description = "Get Downloads By Month",
            parameters = [
                    @Parameter(
                            name = "eventId",
                            in = QUERY,
                            description = "Event Type ID (defaults to 1002 for downloads)",
                            schema = @Schema(implementation = Integer),
                            required = false
                    ),
                    @Parameter(
                            name = "fromMonth",
                            in = QUERY,
                            description = "Starting month in format yyyyMM",
                            schema = @Schema(implementation = String),
                            required = true
                    ),
                    @Parameter(
                            name = "toMonth",
                            in = QUERY,
                            description = "Ending month in format yyyyMM",
                            schema = @Schema(implementation = String),
                            required = true
                    ),
                    @Parameter(
                            name = "excludeReasonTypeId",
                            in = QUERY,
                            description = "Reason type ID to consider as testing",
                            schema = @Schema(implementation = Integer),
                            required = false
                    )
            ],
            responses = [
                    @ApiResponse(
                            description = "Downloads by month in CSV format",
                            responseCode = "200"
                    )
            ]
    )
    @Path("/service/downloadsByMonthCSV")
    @Produces("text/csv")
    def getDownloadsByMonthCSV() {
        if (!params.fromMonth || !params.toMonth) {
            handleError(HttpStatus.BAD_REQUEST, "Request is missing fromMonth and/or toMonth parameters")
            return
        }

        Integer eventId = params.int("eventId") ?: 1002
        Integer excludeReasonTypeId = params.int("excludeReasonTypeId") ?: 10

        def results = loggerService.getDownloadsByMonth(
            eventId,
            params.fromMonth,
            params.toMonth,
            excludeReasonTypeId
        ...
        response.contentType = "text/csv"
        response.addHeader("Content-Disposition", "attachment; filename=\"downloads-by-month-${params.fromMonth}-to-${params.toMonth}.csv\"")

        if (results) {
            def csv = new CSVWriter(response.writer, {
                col1: "month" { it.month }
                col2: "eventTypeId" { it.logEventTypeId }
                col3: "isTesting" { it.isTesting }
                col4: "numUsers" { it.numUsers }
                col5: "numEvents" { it.numEvents }
                col6: "numRecords" { it.numRecords }
            })

            results.each { e -> csv << e }
        } else {
            response.writer.write("\"month\",\"eventTypeId\",\"isTesting\",\"numUsers\",\"numEvents\",\"numRecords\"")
        }
